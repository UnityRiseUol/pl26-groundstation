...
class RocketRotationWidget(gl.GLViewWidget):
    ...
    def set_rotation(self, w, x, y, z):
        # 1. Normalize the incoming quaternion
...
var rocket = L.marker([{LAUNCH_LAT},{LAUNCH_LON}]).addTo(map);
var path = L.polyline([],{{color:'cyan'}}).addTo(map);
...
class MapWidget(QWebEngineView):
    ...
    def update_position(self, lat, lon):
        self.page().runJavaScript(f"window.updateMarker({lat},{lon});")
...
class PLOTSGroundStation(QMainWindow):
    def __init__(self):
        ...
        self.visualizer_selector.addItems(["3D Relative Position", "Rocket Attitude (3D Rotation)", "Live Map Location"])
